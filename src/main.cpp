    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Need Eigen vectors to hold points in vehicle's orientation
          // The size of vectors should be same as ptsx and ptsy
          Eigen::VectorXd ptsx_vehicle(ptsx.size());
          Eigen::VectorXd ptsy_vehicle(ptsy.size());

          // Transform the given waypoints to the vehicle's orientation
          ...
          */
          ...
          // Fit a 3rd-order polynomial to the above x and y coordinates in vehicle's orientation
          auto coeffs = polyfit(ptsx_vehicle, ptsy_vehicle, 3);

          // Calculate the cross track error by evaluating the polynomial function
          // at point px
          double cte = polyeval(coeffs, 0);

          // Calculate the orientation error from the derivative of fit poly line
