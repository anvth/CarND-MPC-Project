...
// Higher value N * dt leads to conservative rides
// Lower value N * dt leads to unstable rides
size_t N = 15;
double dt = 0.12;
...
// The solver receives all the state variables and actuators in a single vector.
// It will be helpful to know, where in the vector each of the variables or
// actuators begin and end.
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Weights to penaize each of the state variables and actuators
    const int cte_cost_weight = 2000;
    const int epsi_cost_weight = 2000;
    const int v_cost_weight = 1;
    const int delta_cost_weight = 2200;
    const int a_cost_weight = 250;
    const int delta_change_cost_weight = 100;
    const int a_change_cost_weight = 10;

    // Penalty for cte, epsi and v
    for (int t = 0; t < N; t++) {
    ...
    // Cost is always at index 0 of `fg`. So we bump up the value of starting indices
    // for all state variables and actuators by 1
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> psi_des_t0 = CppAD::atan(coeffs[1] + 2*coeffs[2]*x_t0 + 3*coeffs[3]*pow(x_t0,2));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // State vector holds all current values
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
